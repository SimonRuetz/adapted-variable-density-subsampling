...
function calculate_norm(W,wf,L,N,f,lines,ind)
...
function generate_l2_dist(W,wf,L,N,f,lines)
    ...
    if lines ==1
        ...
        for ii = 1:N
            ...
            Threads.@threads for j in findall(W.>0)
                ...
                @views Q_current[i] = F_Wav[ lvl[i],:,j[1]]*transpose(F_Wav[lvl[i],ii,j[2]]) * (F_Wav[ lvl[i],:,j[1]]*transpose(F_Wav[lvl[i],ii,j[2]]))' ; #Take the correct wavelets out of the precomputet matrix
                @inline U[i] .+= Q_current[i].*W[j].^2
                #@inline Ut[i][:,(j[2]-1)*N+j[1]] = reshape(Q_current[i]',K,1)
                next!(p)
            end
            U = sum(U)
            d[ii] =opnorm(U-diagm(diag(U)))
            println(ii)
            #println(N)
        end
        d = d/sum(d)
        ...
        for ii1 = 1:S3
            for ii2 = 1:S3
                U = [complex(zeros(S3,S3)) for i = 1:Threads.nthreads()];
                Q_current = [complex(zeros(S3,S3)) for i = 1:Threads.nthreads()];
                Threads.@threads for j in findall(W.>0)
                    i = Threads.threadid();
                    # calculate levels of different wavelets for j_1,j_2
                    n1[i] = Int(L-ceil(log2(j[1]))+1);
                    ...
                    @views Q_current[i] = reshape(F_Wav[ lvl[i],((ii1-1)*S2 + 1) : ii1*S2,j[1]]*transpose(F_Wav[lvl[i],((ii2-1)*S2 + 1) : ii2*S2 ,j[2]]),S2^2,1) * (reshape(F_Wav[ lvl[i],((ii1-1)*S2 +1 ): ii1*S2,j[1]]* transpose(F_Wav[lvl[i],((ii2-1)*S2 +1): ii2*S2 ,j[2]]),S2^2,1))' ; #Take the correct wavelets out of the precomputet matrix
                    @inline U[i] .+= Q_current[i].*W[j].^2
                    #@inline Ut[i][:,(j[2]-1)*N+j[1]] = reshape(Q_current[i]',K,1)
                    next!(p)
                end
                U = sum(U)
                dt[ii1,ii2] =opnorm(U-diagm(diag(U)))
                #dt[ii1,ii2] = maximum(abs.(Q[((ii1-1)*S2 + 1) : ii1*S2,((ii2-1)*S2 + 1) : ii2*S2]))
    ...
    return Dist1, Dist2, Dist3 ,U ,d , dt, QQ
end
