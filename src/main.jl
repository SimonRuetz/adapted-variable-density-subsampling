...
function wavelet_test()
    run_tests(; K=2^16, lk=8.0, flp=0, had=0, lines=0, blocks=0,
...
function line_test()
    run_tests(; K=2^16, lk=59.0, flp=0, had=0, lines=1, blocks=1,
        train_path=joinpath(pwd(), "images", "Brain4"),
        test_path=joinpath(pwd(), "images", "brain4.tif"),
...
function run_tests(;
    ...
    Dist1, Dist2, Dist3, U, d, dt = generate_l2_dist(W, wf, L, N, f, lines)
    ...
    if lines == 1
        ...
        for l in eachindex(d2)
            ...
            Dist2[ll1*In+1:Int64(round(ll1 * In + In)), ll2*In+1:Int64(round(ll2 * In + In))] .= d2[l]
        end
